...
import re
#dows是的chromedriver
chromedriver_path = "D:\pythonproject\36\git_test\python_learn\src\fromnet\chromedriver.exe"
#用ubuntu的chromedriver
# chromedriver_path = "./chromedriver"

doc_dir_path = "D:\python\./doc"
ppt_dir_path = "D:\python\./ppt"
...
class StartChrome():
    ...
    def click_ele(self, click_xpath):
        # 单击指定控件
        click_ele = self.brower.find_element(By.XPATH,click_xpath)
        if click_ele:
    ...
    def create_ppt_doc(self, ppt_dir_path, doc_dir_path):
        ...
        # while True:
        #     # 如果到了最后一页就跳出循环
        #     if self.brower.find_element(By.XPATH,"//div[@class='pagerwg-loadSucc hide']") or self.brower.find_element(By.XPATH,"//div[@class='pagerwg-button' and @style='display: none;']"):
        #         break
        #     # 点击加载更多
    ...
    def create_doc(self, doc_dir_path, sel):
        ...
        # 获取文章内容
        h5 = sel.response.text
        hz = ''.join(re.findall('[\u4e00-\u9fa5]', h5))
        doc = Document()
        ...
        doc.add_paragraph(hz)  # 增加一个paragraph,写入内容
