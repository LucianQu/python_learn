...
def check_events(ship, settings):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:#事件类型为退出
            ...
            key_down_event(ship, event.key, settings)
...
按键按下
"""
def key_down_event(ship, type, settings):
    ...
    elif type == pygame.K_1: #键为1
        #切换飞船图片为鼻涕糖号
        ship.image_type = 1
        ship.image_update = True
    elif type == pygame.K_2:  # 键为2
        # 切换飞船图片为脆糖号
        ship.image_type = 2
        ship.image_update = True
    elif type == pygame.K_3:  # 键为3
        # 切换飞船图片为玉米糖糖号
        ship.image_type = 3
        ship.image_update = True
    elif type == pygame.K_4:  # 键为4
        # 切换飞船图片为酥糖号
        ship.image_type = 4
        ship.image_update = True
    elif type == pygame.K_5:  # 键为5
        # 切换飞船图片为扫地僧糖号
        ship.image_type = 5
        ship.image_update = True
